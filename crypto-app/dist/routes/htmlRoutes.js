...
async function htmlRoutes(server) {
    ...
    server.post('/transactions', async (request, reply) => {
        const client = await server.pg.connect();
        try {
            const { crypto, action, quantity } = request.body;
            ...
            const cryptoResult = await client.query('SELECT id, price FROM cryptocurrencies WHERE symbol = $1', [crypto.toUpperCase()]);
            ...
            const userResult = await client.query('SELECT balance_euro FROM users WHERE id = 1 FOR UPDATE');
            ...
            if (action.toLowerCase() === 'buy') {
                ...
                if (balanceEuro < totalCost) {
                    ...
                    const portfolioResult = await client.query('SELECT amount FROM user_portfolio WHERE user_id = 1 AND crypto_id = $1', [cryptoId]);
                    if (portfolioResult.rows.length > 0) {
                        ...
                        await client.query('UPDATE user_portfolio SET amount = $1 WHERE user_id = 1 AND crypto_id = $2', [newAmount, cryptoId]);
                    }
                    else {
                        // Criptovaluta non presente nel portafoglio, inserisci una nuova riga
                        await client.query('INSERT INTO user_portfolio (user_id, crypto_id, amount) VALUES ($1, $2, $3)', [1, cryptoId, quantity]);
                    }
                }
            }
            ...
            await client.query('INSERT INTO transactions (crypto_id, action, amount, price_at_transaction, status) VALUES ($1, $2, $3, $4, $5)', [cryptoId, action.toLowerCase(), quantity, priceAtTransaction, transactionStatus]);
