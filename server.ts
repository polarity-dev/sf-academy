...
{
  var dataArray = db.prepare("SELECT * FROM data"+(()=>{if(req.query.from) return " WHERE stamp>="+req.query.from; else return ""})()+" ORDER BY stamp"+(()=>{if(req.query.limit) return " LIMIT "+req.query.limit; else return ""})()).all()
  res.json(dataArray)
})
