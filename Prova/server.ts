...
const utenti=db.collection<User>("utenti");
...
app.get('/listUsers', (req, res) => {
  ...
  utenti.findOne<User>({User:username},{projection:{_id:0}}).then(u=>{
    if(u===null) {
...
app.post('/addUser', (req, res) => {
  ...
  utenti.findOne<User>({User:username}).then(u=>{
    if(u!==null){
...
app.post('/login', (req, res) => {
  ...
  utenti.findOne<User>(query).then(u=>{
    if(u!==null){
      res.sendStatus(200);
      return;
    }
