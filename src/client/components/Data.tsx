...
class Data extends React.Component<{}, { from: Dayjs | null, limit: Dayjs | null, data: { result: [{ K: number, D: string }] } | undefined, isLoading: boolean, error: any }> {
		...
		return (<div>
			<Box sx={{
				...
				<Stack justifyContent="center" alignItems="center" direction="row" spacing={2}>
					...
					<Button variant="contained" color="primary" onClick={this.updateTable}>Get Data</Button>
				</Stack>
			</Box>
			<TableContainer component={Paper}>
				<Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
					...
					<TableBody>
						{this.state.data?.result.map((item: { K: number, D: string }, index) => (
							<TableRow
								key={index}
